...
final class UI {
    ...
    public function render_icon( $value, $label = '', $helper_text = '', $attributes = [], $meta = [] ) {
        ...
        <div <?php echo ( $class ) ? 'class="' . esc_attr( $class ) . '"' : ' '; ?>>
            ...
            <div class="mdc-icon-field-img-box <?php if ( $src ) : ?>mdc-with-image<?php endif; ?>">
                ...
                ><?php esc_html_e( 'Icon Library', 'liker' ); ?></button>
                <button class="mdc-icon-field-upload-btn mdc-button mdc-button--outlined mdc-ripple-upgraded"><?php esc_html_e( 'Upload SVG', 'liker' ); ?></button>
            </div>
        ...
        <div class="mdp-dnd">
            ...
            <div class="mdc-text-field mdc-input-width mdc-text-field--outlined mdc-hidden">
                ...
                <input  type="text"
                        ...
                        name="mdp_liker_<?php echo esc_attr( $tab ); ?>_settings[<?php echo esc_attr( $key ); ?>]"
                        id="mdp-liker-settings-dnd-<?php echo esc_attr( $key ); ?>"
                        value="<?php esc_attr_e( $value ); ?>"
                >
                <div class="mdc-notched-outline mdc-notched-outline--upgraded mdc-notched-outline--notched">
                    <div class="mdc-notched-outline__leading"></div>
                    <div class="mdc-notched-outline__notch">
                        <label for="mdp-liker-settings-dnd-<?php echo esc_attr( $key ); ?>" class="mdc-floating-label mdc-floating-label--float-above"><?php echo esc_html( $label ); ?></label>
                    </div>
                    <div class="mdc-notched-outline__trailing"></div>
                </div>
            </div>
            <div id="mdp-<?php echo esc_attr( $key ); ?>-drop-zone" class="<?php if ( $key_exist ) : ?>mdp-key-uploaded <?php endif; ?>mdp-drop-zone">
                <?php if ( $key_exist ) : ?>
                    <span class="material-icons">check_circle_outline</span><?php esc_html_e( 'File exist', 'liker' ); ?>
                    <span class="mdp-drop-zone-hover"><?php esc_html_e( 'Drop file here or click to upload', 'liker' ); ?></span>
                <?php else : ?>
                    <span class="material-icons">cloud</span><?php esc_html_e( 'Drop file here or click to upload.', 'liker' ); ?>
                <?php endif; ?>
            </div>
            <?php if ( $key_exist ) : ?>
                <div class="mdp-messages mdc-text-field-helper-line mdc-text-field-helper-text mdc-text-field-helper-text--persistent">
                    <?php esc_html_e( 'Drag and drop or click on the form to replace file. |', 'liker' ); ?>
                    <a href="#" class="mdp-reset-key-btn"><?php esc_html_e( 'Reset', 'liker' ); ?></a>
                </div>
            <?php else : ?>
                <div class="mdp-messages mdc-text-field-helper-line mdc-text-field-helper-text mdc-text-field-helper-text--persistent">
                    <?php esc_html_e( 'Drag and drop or click on the form to add file', 'liker' ); ?>
                </div>
            <?php endif; ?>
    ...
    private function get_control_id( $key, $tab_slug, $field ) {
        ...
        $id = 'mdp_liker_' . $tab_slug . '_settings_' . $key;
        return apply_filters( 'mdp_liker_settings_control_id', $id, $key, $tab_slug, $field );
    ...
    private function get_control_name( $key, $tab_slug, $field ) {
        ...
        $name = 'mdp_liker_' . $tab_slug . '_settings[' . $key . ']';
        return apply_filters( 'mdp_liker_settings_control_name', $name, $key, $tab_slug, $field );
     ...
     * @param $empty        - value for empty fields 0 or ''
     *
     * @return array
     */
    private function get_sides_values( $options, $slug, $default = array(), $empty = 0 ): array {
        ...
        foreach ( self::$css_sides as $side ) {
            ...
            } else if ( ! isset( $options[ $slug . '_unit' ] ) && ! is_array( $options[ $slug ] ) ) {

                // Migration from old format with single setting for all sides
                $values[ $side ] = $options[ $slug ] ?? $empty;
        ...
        return apply_filters( 'liker_settings_side_values', $values, $slug );
     ...
     * @param $options              - all settings
     * @param $slug                 - slug of the setting
     * @param $default              - default value for unit
     *
     * @return mixed|null
     */
    private function get_sides_unit( $options, $slug, $default = array() ) {

        $unit = $options[ $slug . '_unit' ] ?? $default[ 'unit' ] ?? 'px';

        return apply_filters( 'liker_settings_sides_unit', $unit, $slug );
     ...
     * @param $options      - all settings
     * @param $slug         - slug of the setting
     * @param $default      - default value for linked
     *
     * @return mixed|null
     */
    private function get_sides_linked( $options, $slug, $default = array() ) {

        $linked = $options[ $slug . '_linked' ] ?? $default[ 'linked' ] ?? 'false';

        return apply_filters( 'liker_settings_sides_linked', $linked, $slug );
