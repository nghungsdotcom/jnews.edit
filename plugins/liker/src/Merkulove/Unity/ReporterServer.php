...
final class ReporterServer {
	...
	public function get_title() {
		...
		return esc_html__( 'Server Environment', 'liker' );
	...
	public function get_fields() {
        ...
        $checks = [];
        $checks = $this->add_check( $checks, $server_checks, 'os', esc_html__( 'Operating System', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'software', esc_html__( 'Software', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'mysql_version', esc_html__( 'MySQL version', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'php_version', esc_html__( 'PHP Version', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'write_permissions', esc_html__( 'Write Permissions', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'zip_installed', esc_html__( 'ZIP Installed', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'curl_installed', esc_html__( 'cURL Installed', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'elementor_installed', esc_html__( 'Elementor Installed', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'wpbakery_installed', esc_html__( 'WPBakery Installed', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'allow_url_fopen', esc_html__( 'allow_url_fopen', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'dom_installed', esc_html__( 'DOM Installed', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'xml_installed', esc_html__( 'XML Installed', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'bcmath_installed', esc_html__( 'BCMath Installed', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'mbstring_installed', esc_html__( 'mbstring Installed', 'liker' ) );
        $checks = $this->add_check( $checks, $server_checks, 'server_time', esc_html__( 'Server Time Sync', 'liker' ) );
	...
	public function get_allow_url_fopen() {
		...
		return [
			'value' => $allow_url_fopen ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'liker' ) : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'liker' ),
			'warning' => ! $allow_url_fopen,
			'recommendation' => esc_html__( 'You must enable allow_url_fopen option in PHP. Contact the support service of your hosting provider. They know what to do.', 'liker' )
		];
	...
	public function get_php_version() {
		...
		if ( version_compare( $result['value'], '5.6', '<' ) ) {
			$result['recommendation'] = esc_html__( 'We recommend to use php 5.6 or higher', 'liker' );
	...
	public function get_zip_installed() {
		...
		return [
			'value' => $zip_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'liker') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'liker' ),
	...
	public function get_curl_installed() {
		...
		return [
			'value' => $curl_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'liker' ) : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'liker' ),
			'warning' => ! $curl_installed,
			'recommendation' => esc_html__( 'You must enable CURL (Client URL Library) in PHP. Contact the support service of your hosting provider. They know what to do.', 'liker' )
		];
	...
	public function get_elementor_installed() {
		...
		return [
			'value' => $elementor_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'liker') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'liker' ),
			'warning' => ! $elementor_installed,
			'recommendation' => esc_html__( 'You need install and activate Elementor builder. Go to Elementor site (elementor.com) for details.', 'liker' )
		];
    ...
    public function get_wpbakery_installed() {
        ...
        return [
            'value' => $wpbakery_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'liker') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'liker' ),
            'warning' => ! $wpbakery_installed,
            'recommendation' => esc_html__('You need install and activate WPBakery builder. Go to WPBakery site (wpbakery.com) for details.', 'liker' )
        ];
	...
	public function get_dom_installed() {
		...
		return [
			'value' => $dom_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'liker') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'liker' ),
			'warning' => ! $dom_installed,
			'recommendation' => esc_html__(' You must enable DOM extension (Document Object Model) in PHP. It\'s used for HTML processing. Contact the support service of your hosting provider. They know what to do.', 'liker' )
		];
	...
	public function get_xml_installed() {
		...
		return [
			'value' => $xml_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'liker' ) : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'liker' ),
			'warning' => ! $xml_installed,
			'recommendation' => esc_html__( 'You must enable XML extension in PHP. It\'s used for XML processing. Contact the support service of your hosting provider. They know what to do.', 'liker' )
		];
	...
	public function get_bcmath_installed() {
		...
		return [
			'value' => $bcmath_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'liker' ) : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'liker' ),
			'warning' => ! $bcmath_installed,
			'recommendation' => esc_html__( 'You must enable BCMath extension (Arbitrary Precision Mathematics) in PHP. Contact the support service of your hosting provider. They know what to do.', 'liker' )
		];
    ...
    public function get_mbstring_installed() {
        ...
        return [
            'value' => $mbstring_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'liker' ) : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'liker' ),
            'warning' => ! $mbstring_installed,
            'recommendation' => esc_html__('You must enable mbstring extension (Multibyte String) in PHP. Contact the support service of your hosting provider. They know what to do.', 'liker' )
        ];
    }
    ...
    public function get_server_time() {
        ...
        if ( $curl_errno ) {
            $time_ok = false;
            return [
                'value' => $time_ok ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'liker') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'liker' ),
                'warning' => ! $time_ok,
                'recommendation' => esc_html__('Failed to check time synchronization on your server. Your server\'s clock must be in sync with network time protocol - NTP.', 'liker' )
            ];
        }
        ...
        return [
            'value' => $time_ok ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'liker') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'liker' ) . '<br>Google Time: ' . $google_time . '<br>&nbsp;&nbsp;&nbsp;Local Time: ' . $server_time,
            'warning' => ! $time_ok,
            'recommendation' => esc_html__( ' Your server\'s clock is not in sync with network time protocol - NTP. Contact the support service of your hosting provider. They know what to do.', 'liker' )
        ];
	...
	public function get_write_permissions() {
		...
		$paths_to_check = [
			ABSPATH => esc_html__( 'WordPress root directory', 'liker' )
		];
		...
		if ( $wp_upload_dir[ 'error' ] ) {
			$write_problems[] = esc_html__( 'WordPress root uploads directory', 'liker' );
		}

		$htaccess_file = ABSPATH . '/.htaccess';

		if ( file_exists( $htaccess_file ) ) {
			$paths_to_check[ $htaccess_file ] = esc_html__( '.htaccess file', 'liker' );
		}
		...
		if ( $write_problems ) {
			...
			$value = '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'There are some writing permissions issues with the following directories/files:', 'liker' ) . "<br> &nbsp;&nbsp;&nbsp;&nbsp;– ";
			$value .= implode( "<br> &nbsp;&nbsp;&nbsp;&nbsp;– ", $write_problems );

		} else {

			$value = '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'All right', 'liker' );
