...
final class PluginUpdater {
     ...
     * @var string
	 **/
	private $opt_plugin_info = 'mdp_liker_plugin_info';
	...
	private function __construct() {
		...
		add_action( 'in_plugin_update_message-liker/liker.php', [$this, 'show_plugin_update_message' ], 10, 2 );
	...
	private function compare_with_new( $plugin_info, $wp_version, $update_plugins ) {
		...
		if ( version_compare( $latest_version, $current_version ) > 0 ) {
			...
			$update_plugins->response['liker/liker.php'] = (object)[
				'slug' => 'liker',
                'plugin' => 'liker/liker.php',
	...
	public function plugin_info( $res, $action, $args ) {
		...
		if ( 'liker' !== $args->slug ) { return false; }
	...
	private function get_plugin_information() {
        ...
        if ( ! is_array( $plugin_info ) ) {
	        ...
	        if ( ! $plugin_info->mdp_liker_plugin_info ) {
		        return false;
	        } else {
		        $plugin_info = json_decode( $plugin_info->mdp_liker_plugin_info->body, false );
	        }
        }
	...
	public function show_plugin_update_message( $plugin_data, $r ) {
		...
		if ( ! TabActivation::get_instance()->is_activated() ) {
            ...
            esc_html_e( 'Your plugin license is ', 'liker' );
            echo '<strong style="color: #ff495c">';
            esc_html_e( 'not activated', 'liker' );
            echo '.</strong> ';
            esc_html_e( 'Please activate the license to receive automatic updates in one click, or visit the plugin page on the Envato market to ', 'liker' );
            echo ' <a href="https://1.envato.market/reviewer-elementor" target="_blank">';
			echo ' <a href="https://1.envato.market/liker" target="_blank">';
			esc_html_e( 'download ', 'liker' );
			echo '</a> ';
			esc_html_e( 'the latest version.', 'liker' );
		}
