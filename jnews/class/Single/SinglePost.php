...
Class SinglePost {
	...
	public function get_fullscreen_mode() {
		if ( vp_metabox( 'jnews_single_post.override_template', null, $this->post_id ) ) {
			$enable = vp_metabox( 'jnews_single_post.override.0.fullscreen', false, $this->post_id );
		} else {
	...
	public function get_parallax_mode() {
		if ( vp_metabox( 'jnews_single_post.override_template', null, $this->post_id ) ) {
			$enable = vp_metabox( 'jnews_single_post.override.0.parallax', false, $this->post_id );
		} else {
	...
	public function sticky_sidebar() {
		...
		if ( vp_metabox( 'jnews_single_post.override_template', null, $this->post_id ) ) {
			$sticky_sidebar = vp_metabox( 'jnews_single_post.override.0.sticky_sidebar', false, $this->post_id );
		}
	...
	public function ads_post_enable( $flag, $post_id ) {
		if ( get_post_type() === 'post' && is_single() && $flag ) {
			return ! vp_metabox( 'jnews_single_post.disable_ad', false, $post_id );
		}
		return $flag;
	}
