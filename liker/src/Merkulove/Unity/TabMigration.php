...
final class TabMigration extends Tab {
    ...
    private function __construct() {
        ...
        add_action( 'wp_ajax_export_settings_liker', [ __CLASS__, 'ajax_export_settings' ] );
        add_action( 'wp_ajax_import_settings_liker', [ __CLASS__, 'ajax_import_settings' ] );
	...
	public function add_settings() {
        ...
        if ( ! in_array( Plugin::get_type(), [ 'elementor', 'wpbakery', 'divi' ] ) ) {
            add_settings_field( 'import_settings', esc_html__( 'Import settings:', 'liker' ), [$this, 'render_import_settings'], $group, $section );
            add_settings_field( 'export_settings', esc_html__( 'Export settings:', 'liker' ), [$this, 'render_export_settings'], $group, $section );
        }
		add_settings_field( 'delete_plugin', esc_html__( 'Removal settings:', 'liker' ), [$this, 'render_delete_plugin'], $group, $section );
	...
	public function render_delete_plugin() {
		...
		$uninstall_settings = get_option( 'mdp_liker_migration_settings' );
		...
		/** Prepare description. */
		$helper_text = esc_html__( 'Choose which data to delete upon using the "Delete" action in the "Plugins" admin page.', 'liker' );

		/** Render select. */
		UI::get_instance()->render_select(
			$options,
			$uninstall_settings[ 'delete_plugin' ], // Selected option.
			esc_html__( 'Delete plugin', 'liker' ),
			$helper_text,
			[
			    'name' => 'mdp_liker_migration_settings[delete_plugin]'
            ]
		);
    ...
    public function render_import_settings() {
        ...
        UI::get_instance()->render_import(
            '',
            esc_html__( 'Import settings', 'liker' ),
            esc_html__( 'Drag & Drop file here', 'liker' ),
            'import_settings',
            'migration'
        );
    ...
    public function render_export_settings() {
        ...
        foreach ( $tabs as $slug => $tab ) {
            ...
            UI::get_instance()->render_switcher(
                ...
                $tab['label'] . ' ' . esc_html__( 'Tab', 'liker' ),
                esc_html__( 'Export all settings from', 'liker' ) . ' ' .  $tab['label'] . ' ' . esc_html__( 'tab', 'liker' ),
                [
                    "name" => wp_sprintf(
                        'mdp_liker_%s_settings[export_%s]',
                        self::TAB_SLUG,
                        $slug
                    )
                ]
        ...
        UI::get_instance()->render_button(
            esc_html__( 'Export settings', 'liker' ),
            '',
            'file_download',
            [
                "name" => 'mdp_liker_' . self::TAB_SLUG . '_settings' . "[export_selected]",
                "id" => "mdp-export-btn",
    ...
    public function ajax_export_settings() {
        ...
        foreach ( $options as $opt => $val ) {
            ...
            if ( strpos( $opt,'export_' ) === 0 && $val === 'on' ) {
                ...
                $tab_options = get_option( 'mdp_liker_' . $tab . '_settings' );
    ...
    public function ajax_import_settings() {
        ...
        foreach ( $settings as $tab => $set ) {
            ...
            if ( $set === false ) {
                ...
                // Reset to default
                delete_option( 'mdp_liker_' . $tab . '_settings' );
...
', $setting );
                ...
                // Update settings
                update_option( 'mdp_liker_' . $tab . '_settings', $set );
    ...
    private function import_errors( $e, $plugin = array() ) {
        ...
        switch ( $e ) {
            ...
            case 1:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'No settings for import', 'liker' )
                );
                break;
            case 2:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'Wrong settings format', 'liker' )
                );
                break;
            case 3:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'You are importing settings for', 'liker' ) . ' ' . ucwords( $plugin[ 'slug' ] ) . esc_html__( ', but this is', 'liker' ) . ' ' . Plugin::get_name()
                );
                break;
            case 4:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'You are importing settings for version', 'liker' ) . ' ' . ucwords( $plugin[ 'version' ] ) . ', but the current version is' . ' ' . Plugin::get_version()
                );
                break;
            case 5:
                $message = array(
                    'status' => true,
                    'message' => esc_html__( 'Settings imported', 'liker' )
                );
                break;
            default:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'Unknown error', 'liker' )
                );
